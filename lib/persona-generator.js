...
class PersonaGenerator {
   ...
   * @param {object} [llmConfig] - LLM 配置，提供后启用增强版
   * @param {string} llmConfig.baseUrl - API 地址
   * @param {string} llmConfig.apiKey  - API 密钥
   * @param {string} llmConfig.model   - 模型名
   */
  async generate(index, scenes, pinnedMd, llmConfig) {
    ...
    // 7. LLM 增强（可选）
    let llmEnhancedContent = null;
    if (llmConfig?.baseUrl && llmConfig?.apiKey) {
      try {
        this.#log?.info("[persona] requesting LLM enhancement...");
        llmEnhancedContent = await this.#enhanceWithLlm({
          techProfile, workPatterns, commStyle, problemDomains, lessons,
          totalSessions, totalScenes: scenes.length,
          pinnedMd, scenes,
        }, llmConfig);
        this.#log?.info("[persona] LLM enhancement complete");
      } catch (err) {
        this.#log?.warn(`[persona] LLM enhancement failed, using rule-based: ${err.message}`);
      }
    }

    // 8. 读取旧的 persona 以保留手动编辑的部分
    const existingContent = await this.getPersonaContent();

    // 9. 生成 Markdown
    const baseData = {
      techProfile, workPatterns, commStyle, problemDomains, lessons,
      confidence: llmEnhancedContent ? Math.min(1.0, confidence + 0.1) : confidence,
      totalSessions, totalScenes: scenes.length,
      ...
      pinnedMd, existingContent, llmEnhancedContent,
    };

    const content = this.#buildPersonaMd(baseData);
  ...
  /**
   * 调用 LLM API 增强 Persona 内容
   */
  async #enhanceWithLlm(stats, llmConfig) {
    // 构建 prompt
    const prompt = this.#buildLlmPrompt(stats);
    ...
    const response = await fetch(`${llmConfig.baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "你是一个用户画像分析师。你收到的数据来自一个 AI 助手的对话记录插件，包含了用户的技术使用数据。请基于这些数据提炼出有洞察的用户画像。要求：\n1. 保持客观——只基于给出的数据说话，不要编造\n2. 精简——每段 2-3 句话\n3. 用中文回复\n4. 输出纯 Markdown，不要 JSON\n5. 在最后附一段简短的总结" },
   ...
   * 构建 LLM Prompt
   */
  #buildLlmPrompt(stats) {
    const { techProfile, workPatterns, commStyle, problemDomains, lessons, scenes, pinnedMd } = stats;
    ...
    // 技术倾向
    lines.push("## 工具使用\n");
    lines.push(`常用工具 Top 10：${techProfile.sortedTools.slice(0, 10).map(([t]) => t).join(", ")}`);
    const catLabels = { search: "搜索", fileOps: "文件编辑", exec: "命令执行", system: "系统", plugin: "插件" };
    const cats = Object.entries(techProfile.categoryCounts).filter(([, c]) => c > 0).sort((a, b) => b[1] - a[1]);
    lines.push(`按领域：${cats.map(([c]) => catLabels[c] || c).join(" > ")}`);
    lines.push("");

    // 工作模式
    lines.push("## 工作模式\n");
    lines.push(`活跃时段：${workPatterns.peakTime}`);
    lines.push(`平均对话深度：${commStyle.avgDepth} 轮`);
    lines.push(`典型工具链：${workPatterns.topPatterns.slice(0, 5).map(p => p.pattern).join("; ")}`);
    lines.push("");

    // 问题域
    lines.push("## 主要关注领域\n");
    for (const [domain, count] of problemDomains.sortedDomains) {
      lines.push(`- ${domain}（${count} 次会话）`);
    }
    lines.push("");
    ...
    for (const s of scenes.slice(0, 10)) {
      lines.push(`- ${s.title} [重要性 ${s.importance}/10]`);
    }
    lines.push("");
    ...
    // 置顶记忆
    if (pinnedMd) {
      lines.push("## 用户明确要求记住的内容\n");
      const excerpt = pinnedMd.split("\n").filter(l => l.trim() && !l.startsWith("#") && !l.startsWith("-") && !l.startsWith("---")).slice(0, 15).join("\n").slice(0, 500);
      if (excerpt) lines.push(excerpt + "\n");
    }

    lines.push("\n---\n");
    lines.push("请基于以上数据生成一份用户画像，包含以下章节：");
    lines.push("1. **技术画像**——这个人是什么类型的开发者/用户？（后端？全栈？DevOps？研究者？）");
    lines.push("2. **工作风格**——他喜欢怎么工作？（先调研再动手？直接试？深度探索？广度覆盖？）");
    lines.push("3. **沟通偏好**——他喜欢什么样的交互方式？（简洁直接？详细解释？有来有往的讨论？）");
    lines.push("4. **关注领域**——他主要在做什么方向的工作？");
    lines.push("5. **需要注意的点**——什么情况下他会纠正你？什么类型的错误经常重复？");
    lines.push("6. **一句话总结**——用一句话描述这个用户");
  ...
  #buildPersonaMd(data) {
    ...
    if (llmEnhancedContent) lines.push("enhancedBy: llm");
    lines.push("---");
    lines.push("");
    lines.push("# 用户画像");
    lines.push("");
    ...
    if (llmEnhancedContent) {
      lines.push(`> 已通过 LLM 增强洞察`);
    }
    lines.push("");

    // LLM 增强内容优先
    if (llmEnhancedContent) {
      lines.push(llmEnhancedContent);
      lines.push("");
      lines.push("---");
      lines.push("以下为统计数据（LLM 分析的素材）：");
      lines.push("");
    }
